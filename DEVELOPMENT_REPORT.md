...
mcp-finance/
  ...
  mcp_finance/
    ...
    backtest.py         # Backtrader 回测引擎（5种策略）
...
###  P0 — 已完成
- [x] 策略回测 MCP Tool（Backtrader 事件驱动引擎）
- [x] 选股器 11 维度扩展
- [x] 全市场覆盖（A股+港股+美股+期货）
- [x] 双数据源（easy-tdx + AKShare）
- [x] 单元测试覆盖核心模块
- [x] server.py 完全重构
- [x] 结构化日志 + 统一错误处理
- [x] 线程池重构 + 核心 Bug 修复
...
mcp-finance v0.6.0 完成了从"多数据源实验"到"生产级双数据源平台"的跃升：

1. **双数据源** — easy-tdx (毫秒级行情) + AKShare (全市场覆盖)
2. **线程安全** — 共享线程池消除泄漏，修复 4 处 try/except/finally 结构错误
3. **工具稳定** — 删除不稳定的 search_stock，所有 handler 通过超时保护
4. **质量保障** — 单元测试、结构化日志、参数校验、缓存、统一错误处理
