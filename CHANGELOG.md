## [0.13.1] - 2026-08-01
### 修复
- Dashboard 回测页头部版本号由写死的 `v0.12` 改为动态读取 `mcp_finance.__version__`，并同步 0.13.x 包版本
...
- MCP 工具发现补齐 Walk-Forward 与蒙特卡洛检验，公开工具与 31 个 handler 完全对齐
- MCP 工具 JSON Schema 改由 Pydantic 验证模型生成，避免参数能力与客户端发现结果漂移
- 移除并发调用期间对全局 stdout 的线程级替换，避免 stdio 协议输出竞争
- 修复全市场快照并发等待路径缺少 `time` 导入
- 修复 `600690` 被错误覆盖为中国重工（正确为海尔智家；中国重工为 `601989`）
- 修复回测默认沪深300基准被当作深圳个股取数，导致基准曲线缺失
### 优化
- K线和财务缓存迁移到可配置的用户缓存目录，支持 `MCP_FINANCE_CACHE_DIR`
- 磁盘缓存使用原子替换，避免进程中断留下半写 JSON
- Dashboard 默认仅监听 `127.0.0.1`，局域网访问需显式指定 `--host 0.0.0.0`
- CI 增加未定义名称和重复字典键等运行时缺陷的阻塞检查
